...
import { wowtelligenceLogger, embed, splitChunks } from "./index.ts";
...
export class AgentRuntime implements IAgentRuntime {
    ...
    registerAction(action: Action) {
        wowtelligenceLogger.success(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        wowtelligenceLogger.success(`Normalized action: ${normalizedAction}`);
        ...
        if (!action) {
            wowtelligenceLogger.info("Attempting to find action in similes.");
        ...
        if (!action.handler) {
            wowtelligenceLogger.error(`Action ${action.name} has no handler.`);
            return;
        }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            wowtelligenceLogger.log(`Room ${roomId} created successfully.`);
        }
    }
