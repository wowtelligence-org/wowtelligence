import settings from "@wowtelligence-org/wowtelligence/src/settings.ts";
import {
    IAgentRuntime,
    Memory,
    Provider,
...
export class TokenProvider {
    ...
    constructor(
        ...
        // Find the 'wowtelligence' folder in the filepath and adjust the cache directory path
        const wowtelligenceIndex = __dirname.indexOf("wowtelligence");
        if (wowtelligenceIndex !== -1) {
            const pathToWowtelligence = __dirname.slice(
                0,
                wowtelligenceIndex + 5
            ); // include 'wowtelligence'
            this.cacheDir = path.join(pathToWowtelligence, "cache");
        } else {
