...
case "$1" in
    build)
        docker build --platform linux/amd64 -t wowtelligence .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=wowtelligence)" ]; then
            echo "Container 'wowtelligence' is already running. Stopping it first."
            docker stop wowtelligence
            docker rm wowtelligence
        fi
        ...
        ;;
    start)
        docker start wowtelligence
        ;;
    bash)
        # Check if the container is running before executing bash
        if [ "$(docker ps -q -f name=wowtelligence)" ]; then
            docker exec -it wowtelligence bash
        else
            echo "Container 'wowtelligence' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
